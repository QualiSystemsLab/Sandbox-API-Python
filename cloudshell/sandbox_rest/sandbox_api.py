...
from cloudshell.sandbox_rest.exceptions import (
    SandboxRestAuthException,
...
class SandboxRestApiSession:
    ...
    def __init__(
        ...
        if self.username and self._password:
            self.auth_token = self.get_token_with_credentials(
    ...
    def refresh_auth_from_stored_credentials(self) -> None:
        self.auth_token = self.get_token_with_credentials(
            self.username, self._password, self.domain
        )
        self._auth_headers = self._build_auth_headers(self.auth_token)
    ...
    def _validate_auth_headers(self) -> None:
        if not self._auth_headers:
            raise SandboxRestAuthException(
                f"No auth headers currently set for '{self.username}' session"
            )

    # LOGIN METHODS
    def get_token_with_credentials(
        self, user_name: str, password: str, domain: str
    ) -> str:
        """
        ...
        response = requests.put(
            ...
            data=json.dumps(
                {"username": user_name, "password": password, "domain": domain}
            ),
    ...
    def get_token_for_target_user(self, user_name: str) -> str:
        ...
        if not response.ok:
            raise SandboxRestException(
                f"Failed to get get token for user {user_name}", response
            )
    ...
    def delete_token(self, token_id: str) -> None:
        ...
        response = requests.delete(
            url=f"{self._base_url}/token/{token_id}", headers=self._auth_headers
        )
        if not response.ok:
    ...
    def start_sandbox(
        ...
        sandbox_name = (
            sandbox_name
            if sandbox_name
            else self.get_blueprint_details(blueprint_id)["name"]
        )
        ...
        response = requests.post(
            url, headers=self._auth_headers, data=json.dumps(data_dict)
        )
        if not response.ok:
    ...
    def start_persistent_sandbox(
        ...
        if not response.ok:
            err_msg = (
                f"Failed to start persistent sandbox from blueprint '{blueprint_id}'"
            )
    ...
    def run_sandbox_command(
        ...
        response = requests.post(
            url, data=json.dumps(data_dict), headers=self._auth_headers
        )
        if not response.ok:
            raise SandboxRestException(
                f"failed to start sandbox command '{command_name}'", response
            )
        return response.json()
    ...
    def run_component_command(
        ...
        if not response.ok:
            raise SandboxRestException(
                f"failed to start component command '{command_name}'", response
            )
        return response.json()
    ...
    def extend_sandbox(self, sandbox_id: str, duration: str) -> dict:
        ...
        if not response.ok:
            raise SandboxRestException(
                f"failed to extend sandbox '{sandbox_id}'", response
            )
        return response.json()
    ...
    def stop_sandbox(self, sandbox_id: str) -> None:
        ...
        if not response.ok:
            raise SandboxRestException(
                f"Failed to stop sandbox '{sandbox_id}'", response
            )
    ...
    def get_sandbox_activity(
        ...
        if not response.ok:
            raise SandboxRestException(
                f"Failed to get sandbox activity for '{sandbox_id}'", response
            )
        return response.json()
    ...
    def get_sandbox_commands(self, sandbox_id: str) -> list:
        ...
        if not response.ok:
            raise SandboxRestException(
                f"Failed to get sandbox commands for '{sandbox_id}'", response
            )
        return response.json()
    ...
    def get_sandbox_command_details(self, sandbox_id: str, command_name: str) -> dict:
        ...
        if not response.ok:
            err_msg = (
                f"Failed to get command details for '{command_name}' in '{sandbox_id}'"
            )
            raise SandboxRestException(err_msg, response)
        return response.json()
    ...
    def get_sandbox_component_commands(
        self, sandbox_id: str, component_id: str
    ) -> list:
    ...
    def get_blueprints(self) -> list:
        ...
        self._validate_auth_headers()
        response = requests.get(
            f"{self._versioned_url}/blueprints", headers=self._auth_headers
        )
        if not response.ok:
    ...
    def get_blueprint_details(self, blueprint_id: str) -> dict:
        ...
        if not response.ok:
            raise SandboxRestException(
                f"Failed to get blueprint data for '{blueprint_id}'", response
            )
        return response.json()
    ...
    def get_execution_details(self, execution_id: str) -> dict:
        ...
        if not response.ok:
            raise SandboxRestException(
                f"Failed to get execution details for '{execution_id}'", response
            )
        return response.json()
    ...
    def delete_execution(self, execution_id: str) -> None:
        ...
        if not response.ok:
            raise SandboxRestException(
                f"Failed to delete execution for '{execution_id}'", response
            )
